...
from framedipt.model.layers import (
    Linear,
    flatten_final_dims,
...
def compute_angles(ca_pos: torch.Tensor, pts: torch.Tensor) -> torch.Tensor:
...
class EdgeTransition(nn.Module):
    ...
    def forward(
        self, node_embed: torch.Tensor, edge_embed: torch.Tensor
    ) -> torch.Tensor:
...
class InvariantPointAttention(nn.Module):
    ...
    def forward(  # pylint: disable=R0915
        self,
        s: torch.Tensor,
        z: torch.Tensor,
        r: rigid_utils.Rigid,
        ...
        z_reference_list: list[torch.Tensor] | None = None,
    ) -> torch.Tensor:
        """
        Args:
            s:
        ...
        """
        if offload_inference and z_reference_list is not None:
            z_list = z_reference_list
        else:
            z_list = [z]
        ...
        ##########################
        # [*, N_res, N_res, H]
        b = self.linear_b(z_list[0])

        if offload_inference:
            z_list[0] = z_list[0].cpu()
        ...
        if offload_inference:
            z_list[0] = z_list[0].to(o_pt.device)

        # [*, N_res, H, C_z // 4]
        pair_z = self.down_z(z_list[0]).to(dtype=a.dtype)
        ...
        # [*, N_res, C_s]
        s = self.linear_out(torch.cat(o_feats, dim=-1).to(dtype=z_list[0].dtype))

        return s


class TorsionAngles(nn.Module):
    def __init__(self, c: int, num_torsions: int, eps: float = 1e-8) -> None:
        super().__init__()
...
class ScoreLayer(nn.Module):
    def __init__(self, dim_in: int, dim_hid: int, dim_out: int) -> None:
        super().__init__()
...
class IpaScore(nn.Module):
    def __init__(self, model_conf: DictConfig, diffuser: SE3Diffuser) -> None:
        super().__init__()
    ...
    def scale_pos(self, pos: torch.Tensor) -> torch.Tensor:
        ...
        CA positions are scaled by coordinate_scaling defined in self._ipa_conf.

        Args:
            pos: tensor of translations.

        Returns:
            Scaled translations.
        """
        return pos * self._ipa_conf.coordinate_scaling

    def scale_rigids(self, rigids: rigid_utils.Rigid) -> rigid_utils.Rigid:
        """Scale translations in rigid frames.

        Args:
            rigids: object of Rigid containing rigid frame representations.

        Returns:
            Rigid frames with scaled translations.
        """
        return rigids.apply_trans_fn(self.scale_pos)

    def unscale_pos(self, pos: torch.Tensor) -> torch.Tensor:
        """Unscale translation.

        CA positions are unscaled by coordinate_scaling defined in self._ipa_conf.

        Args:
            pos: tensor of translations.

        Returns:
            Unscaled translations.
        """
        return pos / self._ipa_conf.coordinate_scaling

    def unscale_rigids(self, rigids: rigid_utils.Rigid) -> rigid_utils.Rigid:
        """Unscale translations in rigid frames.

        Args:
            rigids: object of Rigid containing rigid frame representations.

        Returns:
            Rigid frames with unscaled translations.
        """
        return rigids.apply_trans_fn(self.unscale_pos)
    ...
    def forward(
        ...
        curr_rigids = rigid_utils.Rigid.from_tensor_7(torch.clone(init_frames))
        init_rigids = rigid_utils.Rigid.from_tensor_7(init_frames)
